...
namespace Personalities
{
	...
	pub::AI::Personality GetPersonality(const std::string& pilotNickname)
	{
		const auto& pilot = pilots.find(pilotNickname);
		if (pilot == pilots.end())
		{
			// log failure i guess or something lol
			ConPrint(wstring(L"Loading pilot failed! ") + stows(pilotNickname) + L" does not exist!\n");
			return pilot->second;
		}

		ConPrint(wstring(L"Loading pilot success! ") + stows(pilotNickname) + L" exists!\n");
	...
	{
		const std::string str = ToLower(ini.get_value_string(0));
		if (str == "easiest")
			difficulty = 0;
		else if (str == "easy")
			difficulty = 1;
	...
	void LoadJob(INI_Reader& ini)
	{
		...
		while (ini.read_value())
		{
			if (ini.is_value("nickname"))
				...
				data.allow_player_targeting = ToLower(ini.get_value_string()) == "true";
			else if (ini.is_value("force_attack_formation"))
				data.force_attack_formation = ToLower(ini.get_value_string()) == "true";
			else if (ini.is_value("force_attack_formation_used"))
				data.force_attack_formation_used = ToLower(ini.get_value_string()) == "true";
			...
			else if (ini.is_value("field_targeting"))
			{
				...
				data.flee_no_weapons_style = ToLower(ini.get_value_string()) == "true";
			...
			else if (ini.is_value("attack_subtarget_order"))
			{
				...
				for (int i = 0; i <= splitvec.size(); i++)
				{
					if (i == splitvec.size())
					{
						data.attack_subtarget_order[i] = 7;
						continue;
					}
					string s = splitvec[i];
					if (s.find("guns"))
						data.attack_subtarget_order[i] = 0;
					if (s.find("turrets"))
						data.attack_subtarget_order[i] = 1;
					if (s.find("launchers"))
						data.attack_subtarget_order[i] = 2;
					if (s.find("towers"))
						data.attack_subtarget_order[i] = 3;
					if (s.find("engines"))
						data.attack_subtarget_order[i] = 4;
					if (s.find("hull"))
						data.attack_subtarget_order[i] = 5;
					if (s.find("anything"))
						data.attack_subtarget_order[i] = 6;
				}
			}
			else if (ini.is_value("attack_preference"))
			{
				...
				if (shipType == "fighter")
					...
					shipIndex = 5;
				else if (shipType == "tradelane")
					shipIndex = 6;
				else if (shipType == "jumpgate")
					shipIndex = 7;
				else if (shipType == "weapons_platform")
					shipIndex = 8;
				else if (shipType == "destroyable_depot")
					shipIndex = 9;
				...
				pub::AI::Personality::JobStruct::Tattack_order& atkOrder = data.attack_order[attack_preference_ctr];
				atkOrder.distance = ini.get_value_float(1);
				atkOrder.flag = flag;
				atkOrder.type = shipIndex;
				attack_preference_ctr++;
			}
		}

		pub::AI::Personality::JobStruct::Tattack_order endOrder{ 12, 5000, 15 };
		data.attack_order[attack_preference_ctr] = endOrder;
	...
	void LoadPilot(INI_Reader& ini)
	{
		...
		while (ini.read_value())
		{
			if (ini.is_value("nickname"))
				nick = ini.get_value_string();
			if (ini.is_value("inherit"))
			{
				const auto& pilot = pilots.find(ini.get_value_string());
				if (pilot != pilots.end())
				{
					data.GunUse = pilot->second.GunUse;
					data.MissileUse = pilot->second.MissileUse;
					data.EvadeDodgeUse = pilot->second.EvadeDodgeUse;
					data.EvadeBreakUse = pilot->second.EvadeBreakUse;
					data.BuzzHeadTowardUse = pilot->second.BuzzHeadTowardUse;
					data.BuzzPassByUse = pilot->second.BuzzPassByUse;
					data.TrailUse = pilot->second.TrailUse;
					data.StrafeUse = pilot->second.StrafeUse;
					data.EngineKillUse = pilot->second.EngineKillUse;
					data.MineUse = pilot->second.MineUse;
					data.CountermeasureUse = pilot->second.CountermeasureUse;
					data.DamageReaction = pilot->second.DamageReaction;
					data.MissileReaction = pilot->second.MissileReaction;
					data.FormationUse = pilot->second.FormationUse;
					data.RepairUse = pilot->second.RepairUse;
					data.Job = pilot->second.Job;
				}
			}
			if (ini.is_value("gun_id"))
			{
				const auto entity = gun.find(ini.get_value_string());
				if (entity != gun.end())
					data.GunUse = entity->second;
			}
			else if (ini.is_value("missile_id"))
			{
				const auto entity = missile.find(ini.get_value_string());
				if (entity != missile.end())
					data.MissileUse = entity->second;
			}
			else if (ini.is_value("evade_dodge_id"))
			{
				const auto entity = evadeDodge.find(ini.get_value_string());
				if (entity != evadeDodge.end())
					data.EvadeDodgeUse = entity->second;
			}
			else if (ini.is_value("evade_break_id"))
			{
				const auto entity = evadeBreak.find(ini.get_value_string());
				if (entity != evadeBreak.end())
					data.EvadeBreakUse = entity->second;
			}
			else if (ini.is_value("buzz_head_toward_id"))
			{
				const auto entity = buzzHead.find(ini.get_value_string());
				if (entity != buzzHead.end())
					data.BuzzHeadTowardUse = entity->second;
			}
			else if (ini.is_value("buzz_pass_by_id"))
			{
				const auto entity = buzzPass.find(ini.get_value_string());
				if (entity != buzzPass.end())
					data.BuzzPassByUse = entity->second;
			}
			else if (ini.is_value("trail_id"))
			{
				const auto entity = trail.find(ini.get_value_string());
				if (entity != trail.end())
					data.TrailUse = entity->second;
			}
			else if (ini.is_value("strafe_id"))
			{
				const auto entity = strafe.find(ini.get_value_string());
				if (entity != strafe.end())
					data.StrafeUse = entity->second;
			}
			else if (ini.is_value("engine_kill_id"))
			{
				const auto entity = engineKill.find(ini.get_value_string());
				if (entity != engineKill.end())
					data.EngineKillUse = entity->second;
			}
			else if (ini.is_value("mine_id"))
			{
				const auto entity = mine.find(ini.get_value_string());
				if (entity != mine.end())
					data.MineUse = entity->second;
			}
			else if (ini.is_value("countermeasure_id"))
			{
				const auto entity = cm.find(ini.get_value_string());
				if (entity != cm.end())
					data.CountermeasureUse = entity->second;
			}
			else if (ini.is_value("damage_reaction_id"))
			{
				const auto entity = damageReaction.find(ini.get_value_string());
				if (entity != damageReaction.end())
					data.DamageReaction = entity->second;
			}
			else if (ini.is_value("missile_reaction_id"))
			{
				const auto entity = missileReaction.find(ini.get_value_string());
				if (entity != missileReaction.end())
					data.MissileReaction = entity->second;
			}
			else if (ini.is_value("formation_id"))
			{
				const auto entity = formation.find(ini.get_value_string());
				if (entity != formation.end())
					data.FormationUse = entity->second;
			}
			else if (ini.is_value("repair_id"))
			{
				const auto entity = repair.find(ini.get_value_string());
				if (entity != repair.end())
					data.RepairUse = entity->second;
			}
			else if (ini.is_value("job_id"))
			{
				const auto entity = job.find(ini.get_value_string());
				if (entity != job.end())
	...
	void LoadPersonalities()
	{
		...
		{
			ConPrint(L"Unable to parse pilot_population\n");
			return;
		}

		ConPrint(L"Parsing Pilot Population\n");
		...
		ConPrint(wstring(L"Parsed Pilot Population: ") + to_wstring(pilots.size()) + L" personalities\n");
	}
